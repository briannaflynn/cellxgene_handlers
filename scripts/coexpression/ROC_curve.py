...
#Date: May 22, 2024
#Author: Muyoung Lee
#Description: Draw ROC curve of sorted gene list with well-known biological complexes
...
# Functions
def cal_TPR_FPR_AUC(sorted_gene_list, positive_ctrl_set):
...
def draw_curve(coordinates_for_graph, complex_name):
	x = []
	y = []
...
def main():
	...
	with open("random_gene_sets.txt") as RANDOM:
		...
		for line in RANDOM:
			...
			geneset_dict[f"Random_set{count}"] = set(EnsID_list)
	...
	print("Query_gene\tGeneset\tROC")	
	for known_complex in geneset_dict:
		values = cal_TPR_FPR_AUC(sorted_gene_list, geneset_dict[known_complex])
		draw_curve(values, known_complex)
	...
	plt.savefig(f"{target_gene}.ROC.png", dpi=300)
	plt.clf()	
